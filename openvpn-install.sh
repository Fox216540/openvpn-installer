...
#
# Modified work:
#   Copyright (c) 2025 Fox216540
#   Licensed under the MIT License
#
# License notice:
#   This script contains code under the MIT License from both authors
#   (Nyr and Fox216540). See LICENSE file for full license text.
#
...
management 127.0.0.1 7505" > /etc/openvpn/server/server.conf
	# IPv6
	if [[ -z "$ip6" ]]; then
	...
	conn_inter="${CONNECTIONS_INTERVAL_SH:-10}"
...
EOF
        ...
        cleanup() {
                      echo "ВНИМАНИЕ: каталог $WORK_DIR не удалён (отладка)"
                      # rm -rf "$WORK_DIR"
                  }
        trap cleanup EXIT
